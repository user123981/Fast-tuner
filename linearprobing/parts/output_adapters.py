...
from .multimae_utils import (
    ...
    build_2d_sincos_posemb, build_3d_sincos_posemb,
    pair, triplet,
    trunc_normal_
)
...
class VolumetricOutputAdapter(nn.Module):
    """Cross-attention adapter for spatial outputs, like videos or volumetric data.
    ...
    def __init__(self,
        ...
        if not self.learnable_pos_emb:
            print("> Building 3D sincos posemb")
            ...
            print("> Building learnable posemb")
            self.pos_emb = nn.Parameter(torch.zeros(1, t_posemb, h_posemb, w_posemb, self.dim_tokens))
        ...
        self.dim_patch = self.num_channels * self.P_T * self.P_H * self.P_W
    ...
    def generate_context_embeddings(self, input_info,
        ...
        for task, info in input_info["tasks"].items():
            ...
            if info['has_posemb']:
                curr_size = input_info['tasks'][task]['grid_size']
                pos_emb = F.interpolate(self.pos_emb, curr_size, mode='trilinear', align_corners=False)
                pos_emb = rearrange(pos_emb, 'b d nt nh nw -> b (nt nh nw) d')
                assert info['num_tokens'] == pos_emb.shape[1], (
                    'VolumetricOutputAdapter:'
                    f" Number of tokens {info['num_tokens']} does not match"
                    f" number of positional embeddings {pos_emb.shape[1]}"
                )
    ...
    def get_queries_and_context(self, context_tokens, input_info, ids_keep, ids_restore):
        ...
        T, H, W = input_info['tasks'][self.task]['image_size']
        ...
        context_emb = self.generate_context_embeddings(input_info=input_info, bs=B, size=(N_T, N_H, N_W),
        ...
        if self.use_task_queries and self.task in input_info['tasks']:
            ...
            queries = repeat(self.mask_token, '() () d -> b n d', b=B, n=N_T * N_H * N_W)
            queries_pos_emb = F.interpolate(self.pos_emb, size=(N_T, N_H, N_W), mode='bilinear', align_corners=False)
            ...
            if self.task_embeddings is not None and self.task in self.task_embeddings:
                queries_task_emb = repeat(self.task_embeddings[self.task], '() () d -> b n d', b=B, n=N_T * N_H * N_W)
    ...
    def forward(self,
        ...
        x = rearrange(
            ...
            nt=N_T, nh=N_H, nw=N_W, pt=self.P_T, ph=self.P_H, pw=self.P_W, c=self.num_channels
        )
