...
from src.VisionFM_main.utils import trunc_normal_
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 init_values=0, use_mean_pooling=False, masked_im_modeling=False, feat_concat=False, use_flash=False, use_norm=True):
        super().__init__()
    ...
    def forward(self, x, return_all_tokens=None, mask=None, maps=None):
        ...
        if self.use_norm:
            x = self.norm(x) # need this for baseline. commented this out.     return [2*B, 197, 384], two global crops
        ...
        if self.feat_concat: # default in Sp16: False
            feats = x[:, 1:].mean(dim=1)
            concats = torch.cat((x[:, 0], feats), dim=1)
            return self.head(concats)
