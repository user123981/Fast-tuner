...
from .multimae_utils import (
    build_2d_sincos_posemb, pair, trunc_normal_, triplet, build_3d_sincos_posemb
)
...
class Patched3DInputAdapter(nn.Module):
    """Adapter for 3D inputs, like videos or volumetric data.
    Creates tokens from patches over the 3D input.
    ...
    def __init__(self,
                 num_channels: int,
                 stride_level: int,
                 patch_size_full: Union[int, Tuple[int,int,int]],
                 dim_tokens: Optional[int] = None,
                 sincos_pos_emb: bool = True,
                 learnable_pos_emb: bool = False,
                 image_size: Union[int, Tuple[int,int,int]] = 224):

        super().__init__()
        self.num_channels = num_channels
        self.stride_level = stride_level
        self.patch_size_full = triplet(patch_size_full)
        self.dim_tokens = dim_tokens
        self.sincos_pos_emb = sincos_pos_emb
        self.learnable_pos_emb = learnable_pos_emb
        ...
        # print(f'Image size: {self.image_size}, Patch size: {self.patch_size_full}')
        self.num_patches = \
            (self.image_size[0] // self.patch_size_full[0]) \
            * (self.image_size[1] // self.patch_size_full[1]) \
            * (self.image_size[2] // self.patch_size_full[2])

        assert len(self.patch_size_full) == 3, 'Patch size must be a tuple of 3 ints'
        # Actual patch height and width, taking into account stride of input
        self.P_T = max(1, self.patch_size_full[0] // stride_level)
        self.P_H = max(1, self.patch_size_full[1] // stride_level)
        self.P_W = max(1, self.patch_size_full[2] // stride_level)
    ...
    def init(self, dim_tokens: int = 768):
        ...
        # Task embedding identifying from which task a given token comes from
        # Fixed-size positional embeddings. Can be interpolated to different input sizes
        t_posemb = self.image_size[0] // (self.stride_level * self.P_T)
        h_posemb = self.image_size[1] // (self.stride_level * self.P_H)
        w_posemb = self.image_size[2] // (self.stride_level * self.P_W)
        if self.sincos_pos_emb:
            self.pos_emb = build_3d_sincos_posemb(t=t_posemb, h=h_posemb, w=w_posemb, embed_dim=self.dim_tokens)
            self.pos_emb = nn.Parameter(self.pos_emb, requires_grad=self.learnable_pos_emb)
        else:
            self.pos_emb = nn.Parameter(torch.zeros(1, self.dim_tokens, t_posemb, h_posemb, w_posemb))
            trunc_normal_(self.pos_emb, std=0.02)

        # Image -> tokens projection
        self.proj = nn.Conv3d(
            in_channels=self.num_channels, out_channels=self.dim_tokens,
            kernel_size=(self.P_T, self.P_H, self.P_W), stride=(self.P_T, self.P_H, self.P_W)
        )
    ...
    def forward(self, x):
        ...
        B, C, T, H, W = x.shape
        assert self.dim_tokens is not None, 'Need to call init(dim_tokens) function first'
        assert (T % self.P_T == 0) and (H % self.P_H == 0) and (W % self.P_W == 0), \
            f'Image sizes {T}x{H}x{W} must be divisible by patch sizes {self.P_T}x{self.P_H}x{self.P_W}'
        N_T, N_H, N_W = T // self.P_T, H // self.P_H, W // self.P_W # Number of patches in height and width

        # Create patches [B, C, T, H, W] -> [B, (T*H*W), C]
        x_patch = rearrange(self.proj(x), 'b d nt nh nw -> b (nt nh nw) d')

        # Create positional embedding
        x_pos_emb = F.interpolate(self.pos_emb, size=(N_T, N_H, N_W), mode='trilinear', align_corners=False)
...
class TestPatched3DInputAdapter(unittest.TestCase):
    def test_2d(self):
        # Create 2D input adapter
        ...
        # Forward pass
        y = adapter(x)
        self.assertEqual(y.shape, (2, 1024, 768))

    def test_3d(self):
        # Create 3D input adapter
        adapter = Patched3DInputAdapter(
            num_channels=1,
            stride_level=1,
            patch_size_full=(8, 16, 16),
            dim_tokens=768,
            sincos_pos_emb=True,
            learnable_pos_emb=False,
            image_size=(128, 512, 512)
        )

        # Create random input tensor
        x = torch.randn(2, 1, 128, 512, 512)

        # Forward pass
        y = adapter(x)
        self.assertEqual(y.shape, (2, 16384, 768))
