        ...
        if index in self.cache:
            ...
            for task in self.tasks:
                ...
                if path.endswith('.npy') or path.endswith('.npz'):
                    ...
                    if False:
                    # if sample.shape != tuple(self.args.input_size[task]):
                        # if 'layermap' in task:
                        #     order = 0
                        # else:
                        ...
                        # else:
                        #     order = 0
                        center_crop_from = self.args.center_crop_from
                        if center_crop_from is not None:
                            target_size = pair(center_crop_from)
                        else:
                            target_size = self.args.input_size[task]
                        order=1
                        ...
                        if center_crop_from is not None:
                            if start_i is None and start_j is None:
                                start_i = random.randint(0, sample.shape[0] - self.args.input_size[task][0])
                                start_j = random.randint(0, sample.shape[1] - self.args.input_size[task][1])
                                # print(f"Using start_i {start_i} and start_j {start_j}")
                            sample = sample[
                                start_i:start_i + self.args.input_size[task][0],
                                start_j:start_j + self.args.input_size[task][1]
                            ]
                            # print(f"Resized {task} to {sample.shape}")

                        # print(sample.shape)
                    # Both layers and oct have 3 dimensions
                    #  - OCT: (num_slices, height, width)
                    #  - LAYERS: (num_slices, width, num_layers)
                    # if sample.ndim == 3:
                    #     if slice is None:
                    #         num = np.random.normal(loc=0.5, scale=0.2, size=1)
                    #         num = np.clip(num, 0.0, 1.0)
                    #         slice = int(num * sample.shape[0]) - 1
                    #     sample = sample[slice, :, :]
                    # if task == 'layermaps':
                        # print(sample.shape, sample.min(), sample.max())
                        # One-hot encode the layer maps
                        # sample = (np.arange(13) == sample[...,None]-1).astype(np.float32).transpose(2, 0, 1)
                        # pass
